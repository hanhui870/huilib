...
class XssFilter extends \HuiLib\Module\ModuleBase
{
	...
	public static function filter($string)
	{
		...
		while ( preg_match ( $searchHexEncodings, $string ) || preg_match ( $searchUnicodeEncodings, $string ) ) {
			$string = preg_replace ( $searchHexEncodings, "chr(hexdec('\\1'))", $string );
			$string = preg_replace ( $searchUnicodeEncodings, "chr('\\1')", $string );
		}
		...
		$string2 = preg_replace ( '/(&#[xX]?0{0,8}(9|10|13|a|b);)*\s*/i', '', $string );
		$ra = array ();
		...
		foreach ( $ra1 as $ra1word ) {
			...
			if (stripos ( $string2, $ra1word ) !== false) {
